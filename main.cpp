...
int main(){
    ...
    while (true){
        ...
        NewFunction(stack, que, fList, dList, tree, arr);
        }
...
void NewFunction(Stack<std::string> &stack, queue<std::string> &que, FList<std::string> &fList, DList<std::string> &dList, fullBinary<int> &tree, MyArray<std::string> &arr)
{
    clearStack(&stack);
            clearQueue(&que);
            clearFList(&fList);
